...
# ── Feature extraction ────────────────────────────────────────────────────────
# Two-staged model (prof's format) uses:
#   Stage 1 — 25 features: mean, std, min, max, range  ×  5 flex channels
#   Stage 2 — 29 features: same 25 flex features + mean(qw,qx,qy,qz)
# Legacy single-model format uses 45 features (5 stats × 9 channels).
...
def extract_features_stage1(window: np.ndarray) -> np.ndarray:
    """25 features — flex channels 0-4 only."""
    feats = []
    for i in range(5):
